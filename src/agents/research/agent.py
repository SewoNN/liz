...
from src.agents.research.prompts import SYSTEM_PROMPT
from src.agents.research.tools import TOOLS
...
class ResearchAgent(BaseReActAgent):
    ...
    def __init__(self):
        ...
        super().__init__(
            name="Research Agent",
            description="An agent that researches and analyzes information based on user queries.",
            tools=TOOLS,
    ...
    async def call_model(self, state: AgentState, config: RunnableConfig) -> Dict[str, List[AIMessage]]:
        ...
        if response.tool_calls:
            structured_response = {
                ...
                "final_report": "Processing with tools..."
            }
        ...
        try:
            ...
            if not isinstance(parsed_response, dict) or "chain_of_thought" not in parsed_response or "final_report" not in parsed_response:
                parsed_response = {
